...
class JDService(object):
    ...
    def __init__(self):
        ...
        self.mobile_num = global_config.get('config', 'mobile_num')
        ...
        self.order_leading_in_millis = float(global_config.get('config', 'order_leading_in_millis'))
        ...
        self.enable_mobile_client = False
    ...
    def _assign_cookies_from_remote(self, cookies, is_pc_cookie=True):
    ...
    def load_login_cookie(self, login_username, cookie_token):
        ...
        jd_user_data = self.user_service.find_jd_user_by_nick_name(self.nick_name)
    ...
    def _need_auth_code(self, username):
        ...
        if not response_status(resp):
            self.log_stream_error('获取是否需要验证码失败')
            return False
    ...
    def login_by_username(self):
        if self.is_login:
            self.log_stream_info('登录成功')
            return True

        username = input('账号:')
        password = input('密码:')
        if (not username) or (not password):
            self.log_stream_error('用户名或密码不能为空')
            return False
        ...
        if self._need_auth_code(username):
            self.log_stream_info('本次登录需要验证码')
            auth_code = self._get_auth_code(uuid)
        else:
            self.log_stream_info('本次登录不需要验证码')
    ...
    def _get_login_result(self, resp):
        ...
        if 'success' in resp_json:
            ...
            error_msg = resp_json
        self.log_stream_error(error_msg)
        return False
    ...
    def login_by_QRcode(self):
        ...
        try:
            ...
            self.log_stream_info('未登录，需要扫码登录')
        ...
        for _ in range(retry_times):
            ...
            raise RestfulException(error_dict['SERVICE']['JD']['QR_EXPIRED'])

        # validate QR code ticket
        if not self._validate_QRcode_ticket(ticket):
            raise RestfulException(error_dict['SERVICE']['JD']['QR_INVALID'])

        self.log_stream_info('二维码登录成功')
        self.is_login = True
        self.nick_name = self.get_user_info()
        self.log_stream_info(self.nick_name)
        self._save_cookies()
    ...
    def mobile_login(self, login_username, nick_name, mobile_num):
        ...
        try:
            ...
            if mobile_nick_name:
                ...
                jd_user_data = self.user_service.find_jd_user_by_nick_name(nick_name)
    ...
    def check_individual_client_cookie(self):
        try:
            ...
            try:
                ...
                self.is_login = False
                self.log_stream_error('测试电脑端cookie测试有效性失败：%s', str(e))
                self.messenger.send(text='用户' + self.nick_name + '电脑端cookie测试有效性失败', desp='请重新扫码登录')
                raise e

        # 如果手机端开启，验证cookie
        if self.enable_mobile_client:
            ...
            if mobile_nick_name:
                ...
                if mobile_nick_name:
                    ...
                    if not self.mobile_login(self.mobile_num):
    ...
    @fetch_latency
    def upodate_sys_time(self, target_time, leading_in_sec):
        self.log_stream_info('抢购开始前%s秒更新系统时间', leading_in_sec)
        adjust_server_time(self.analyse_server_time)
    ...
    def sync_target_time(self, target_time, leading_in_sec, is_adjust_time, is_warm_time_request):
        ...
        if is_adjust_time:
            ...
            target_time_datetime = datetime_offset_in_milliesec(target_time_datetime, -self.order_leading_in_millis)

            target_time = datetime_to_str(target_time_datetime)
            self.log_stream_info('调整后的抢购时间                     %s', target_time)

        return target_time, t_server_real_time, t_reach
            ...
            for unpaid_order_item in order_list:
                ...
                for submitted_order_id  in order_id_list:
                    if str(submitted_order_id) == unpaid_order_id:
                        ...
                        elif is_send_message and self.send_message:
                            text, desp = build_order_message(self.nick_name, unpaid_order_item)
                            self.messenger.send(text=text, desp=desp)
    ...
    def save_order(self, order_id):
        ...
        for unpaid_order_item in order_list:
            ...
            if str(order_id) == unpaid_order_id:
                ...
                self.user_service.save_jd_order(self.login_username, unpaid_order_item)
    ...
    def actions_before_target_time(self, target_time):
        ...
        if os.name == 'nt':
            leading_in_sec = random.randint(20, 50)
            sleep_interval = 0.0001
            title = '抢购前[{0}]秒更新系统时间'.format(leading_in_sec)
            self.call_function_with_leading_time(title, sleep_interval, self.upodate_sys_time, target_time, leading_in_sec)
    ...
    def prepare_on_init(self, target_time, sku_id, num):
        try:
            ...
            # 测试cookie有效性
            if self.mobile_num:
                self.enable_mobile_client = True
                self._load_mobile_cookies()
